...
- 🧩 **Multi-lane composition** — declare additional Runtimes and Gateways in `additional_runtimes` and `additional_gateways`; disabled entries create nothing, and every Runtime chooses whether the module creates or reuses its execution role.
- 🛡️ **Shared policy enforcement** — opt in to one Policy Engine, attach it independently to each Gateway, and render caller-owned Cedar or Dogwood templates against stable Gateway keys.
- 🌐 **Browser and evaluations** — opt in from the root module when those services belong to the same deployment, or continue using the focused submodules independently.
...
AgentCore services remain opt-in. The root module can compose multiple Runtime
and Gateway lanes with Memory, Browser, Code Interpreter, Policy, Evaluations,
and built-in Gateway connectors. Focused submodules remain available when a
service has a deliberately separate lifecycle. See
[`multi-runtime-governance`](examples/multi-runtime-governance) for the composed
form; the basic example remains intentionally small.
...
```
.
├── main.tf          # Root module — composes only explicitly enabled resources
├── variables.tf
├── outputs.tf
├── versions.tf
└── modules/
...
| Additional Runtime and execution-role resources | An `additional_runtimes` entry has `enabled = true` | Creates independently authorized Runtime lanes; each entry explicitly creates or reuses its IAM role. |
| Additional Gateway, role, and target resources | An `additional_gateways` entry has `enabled = true` | Creates independently authorized Gateway lanes and optionally attaches a selected Runtime. |
| `aws_bedrockagentcore_policy_engine` | `create_policy_engine = true` | Shared Policy Engine that enabled Gateways may attach in `LOG_ONLY` or `ENFORCE` mode. |
| `aws_bedrockagentcore_policy` | `gateway_policy_templates` is non-empty | Caller-owned Cedar policies rendered with the selected Gateway ARN. |
| `aws_cloudformation_stack` (Dogwood policy) | `temporal_policy_templates` is non-empty | Isolated lifecycle for Dogwood policy definitions until the AWS Provider exposes their native schema. |
| `aws_bedrockagentcore_browser` | `create_browser = true` | Managed Browser with optional profiles and Runtime IAM access. |
| Evaluator and online evaluation resources | `create_evaluations = true` | Evaluators and sampled online evaluation configurations, optionally derived from a Runtime key. |
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_additional_gateway"></a> [additional\_gateway](#module\_additional\_gateway) | ./modules/gateway | n/a |
| <a name="module_additional_gateway_runtime_target"></a> [additional\_gateway\_runtime\_target](#module\_additional\_gateway\_runtime\_target) | ./modules/gateway-target | n/a |
| <a name="module_additional_runtime"></a> [additional\_runtime](#module\_additional\_runtime) | ./modules/runtime | n/a |
...
| [aws_iam_role.additional_runtime](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role) | resource |
| [aws_iam_role.agent_execution](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role) | resource |
| [aws_iam_role_policy.additional_gateway_runtime_invoke](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
| [aws_iam_role_policy.additional_runtime](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
...
| [aws_iam_role_policy_attachment.additional_runtime](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
...
| [terraform_data.composition_validations](https://registry.terraform.io/providers/hashicorp/terraform/latest/docs/resources/data) | resource |
...
|------|-------------|------|---------|:--------:|
| <a name="input_additional_gateways"></a> [additional\_gateways](#input\_additional\_gateways) | Additional opt-in Gateway instances managed by this root module. Set runtime\_key to attach the primary Runtime or an enabled additional Runtime. | <pre>map(object({<br/>    enabled          = optional(bool, true)<br/>    name             = string<br/>    description      = optional(string)<br/>    create_role      = optional(bool, true)<br/>    role_arn         = optional(string)<br/>    role_policy_arns = optional(set(string), [])<br/>    role_policy_statements = optional(list(object({<br/>      sid       = optional(string)<br/>      effect    = optional(string, "Allow")<br/>      actions   = set(string)<br/>      resources = set(string)<br/>      condition = optional(any)<br/>    })), [])<br/>    authorizer_type = optional(string, "AWS_IAM")<br/>    authorizer_configuration = optional(object({<br/>      discovery_url            = string<br/>      allowed_audience         = optional(set(string), [])<br/>      allowed_clients          = optional(set(string), [])<br/>      allowed_scopes           = optional(set(string), [])<br/>      workload_identities      = optional(list(string), [])<br/>      hosting_environment_arns = optional(list(string), [])<br/>      custom_claims = optional(set(object({<br/>        inbound_token_claim_name       = string<br/>        inbound_token_claim_value_type = string<br/>        claim_match_operator           = string<br/>        match_value_string             = optional(string)<br/>        match_value_string_list        = optional(set(string))<br/>      })), [])<br/>      private_endpoint = optional(object({<br/>        managed_vpc_resource = optional(object({<br/>          endpoint_ip_address_type = string<br/>          subnet_ids               = set(string)<br/>          vpc_identifier           = string<br/>          routing_domain           = optional(string)<br/>          security_group_ids       = optional(set(string), [])<br/>          tags                     = optional(map(string), {})<br/>        }))<br/>        self_managed_lattice_resource = optional(object({<br/>          resource_configuration_identifier = string<br/>        }))<br/>      }))<br/>      private_endpoint_overrides = optional(list(object({<br/>        domain = string<br/>        private_endpoint = object({<br/>          managed_vpc_resource = optional(object({<br/>            endpoint_ip_address_type = string<br/>            subnet_ids               = set(string)<br/>            vpc_identifier           = string<br/>            routing_domain           = optional(string)<br/>            security_group_ids       = optional(set(string), [])<br/>            tags                     = optional(map(string), {})<br/>          }))<br/>          self_managed_lattice_resource = optional(object({<br/>            resource_configuration_identifier = string<br/>          }))<br/>        })<br/>      })), [])<br/>    }))<br/>    protocol_type = optional(string)<br/>    protocol_configuration = optional(object({<br/>      instructions               = optional(string)<br/>      search_type                = optional(string)<br/>      supported_versions         = optional(set(string), [])<br/>      session_timeout_in_seconds = optional(number)<br/>      enable_response_streaming  = optional(bool)<br/>    }))<br/>    policy_engine_mode = optional(string)<br/>    interceptor_configurations = optional(list(object({<br/>      interception_points  = list(string)<br/>      lambda_arn           = string<br/>      pass_request_headers = optional(bool, false)<br/>    })), [])<br/>    targets     = optional(any, {})<br/>    runtime_key = optional(string)<br/>    runtime_target = optional(object({<br/>      name                              = optional(string)<br/>      description                       = optional(string)<br/>      region                            = optional(string)<br/>      qualifier                         = optional(string, "DEFAULT")<br/>      credential_provider_configuration = optional(any, { gateway_iam_role = { service = "bedrock-agentcore" } })<br/>      metadata_configuration = optional(object({<br/>        allowed_query_parameters = optional(set(string), [])<br/>        allowed_request_headers  = optional(set(string), [])<br/>        allowed_response_headers = optional(set(string), [])<br/>      }))<br/>      private_endpoint = optional(any)<br/>      timeouts = optional(object({<br/>        create = optional(string)<br/>        update = optional(string)<br/>        delete = optional(string)<br/>      }))<br/>    }), {})<br/>    resource_policy_role_arns = optional(set(string))<br/>    kms_key_arn               = optional(string)<br/>    exception_level           = optional(string)<br/>    region                    = optional(string)<br/>    timeouts = optional(object({<br/>      create = optional(string)<br/>      update = optional(string)<br/>      delete = optional(string)<br/>    }))<br/>    tags = optional(map(string), {})<br/>  }))</pre> | `{}` | no |
...
| <a name="input_additional_runtimes"></a> [additional\_runtimes](#input\_additional\_runtimes) | Additional opt-in Runtime instances managed by this root module. Disabled entries are ignored. Execution roles remain explicit per Runtime. | <pre>map(object({<br/>    enabled                     = optional(bool, true)<br/>    name                        = string<br/>    description                 = optional(string, "Managed by terraform-aws-agentcore.")<br/>    create_execution_role       = optional(bool, false)<br/>    execution_role_arn          = optional(string)<br/>    additional_iam_statements   = optional(list(any), [])<br/>    additional_iam_policy_arns  = optional(set(string), [])<br/>    allow_bedrock_invoke_all    = optional(bool, false)<br/>    allow_user_id_access_tokens = optional(bool, false)<br/>    memory_access_enabled       = optional(bool, false)<br/>    browser_access_enabled      = optional(bool, false)<br/>    image_uri                   = optional(string)<br/>    image_digest                = optional(string)<br/>    runtime_code_configuration = optional(object({<br/>      entry_point = list(string)<br/>      runtime     = string<br/>      s3 = object({<br/>        bucket     = string<br/>        prefix     = string<br/>        version_id = optional(string)<br/>      })<br/>    }))<br/>    runtime_filesystems = optional(list(object({<br/>      session_storage = optional(object({<br/>        mount_path = string<br/>      }))<br/>      s3_files_access_point = optional(object({<br/>        access_point_arn = string<br/>        mount_path       = string<br/>      }))<br/>      efs_access_point = optional(object({<br/>        access_point_arn = string<br/>        mount_path       = string<br/>      }))<br/>    })), [])<br/>    network_mode           = optional(string, "PUBLIC")<br/>    vpc_security_group_ids = optional(list(string), [])<br/>    vpc_subnet_ids         = optional(list(string), [])<br/>    authorizer_configuration = optional(object({<br/>      discovery_url            = string<br/>      allowed_audience         = optional(set(string), [])<br/>      allowed_clients          = optional(set(string), [])<br/>      allowed_scopes           = optional(set(string), [])<br/>      workload_identities      = optional(list(string), [])<br/>      hosting_environment_arns = optional(list(string), [])<br/>      custom_claims = optional(set(object({<br/>        inbound_token_claim_name       = string<br/>        inbound_token_claim_value_type = string<br/>        claim_match_operator           = string<br/>        match_value_string             = optional(string)<br/>        match_value_string_list        = optional(set(string))<br/>      })), [])<br/>      private_endpoint = optional(object({<br/>        managed_vpc_resource = optional(object({<br/>          endpoint_ip_address_type = string<br/>          subnet_ids               = set(string)<br/>          vpc_identifier           = string<br/>          routing_domain           = optional(string)<br/>          security_group_ids       = optional(set(string), [])<br/>          tags                     = optional(map(string), {})<br/>        }))<br/>        self_managed_lattice_resource = optional(object({<br/>          resource_configuration_identifier = string<br/>        }))<br/>      }))<br/>      private_endpoint_overrides = optional(list(object({<br/>        domain = string<br/>        private_endpoint = object({<br/>          managed_vpc_resource = optional(object({<br/>            endpoint_ip_address_type = string<br/>            subnet_ids               = set(string)<br/>            vpc_identifier           = string<br/>            routing_domain           = optional(string)<br/>            security_group_ids       = optional(set(string), [])<br/>            tags                     = optional(map(string), {})<br/>          }))<br/>          self_managed_lattice_resource = optional(object({<br/>            resource_configuration_identifier = string<br/>          }))<br/>        })<br/>      })), [])<br/>    }))<br/>    trusted_gateway_keys         = optional(set(string), [])<br/>    idle_runtime_session_timeout = optional(number)<br/>    max_lifetime                 = optional(number)<br/>    server_protocol              = optional(string)<br/>    request_header_allowlist     = optional(list(string), [])<br/>    environment_variables        = optional(map(string), {})<br/>    region                       = optional(string)<br/>    timeouts = optional(object({<br/>      create = optional(string)<br/>      update = optional(string)<br/>      delete = optional(string)<br/>    }))<br/>    resource_policy_configuration = optional(object({<br/>      role_arns    = optional(set(string), [])<br/>      gateway_keys = optional(set(string), [])<br/>    }))<br/>    tags = optional(map(string), {})<br/>  }))</pre> | `{}` | no |
...
| <a name="input_browser_description"></a> [browser\_description](#input\_browser\_description) | Description of the root Browser. | `string` | `null` | no |
...
| <a name="input_browser_name"></a> [browser\_name](#input\_browser\_name) | Name of the root Browser. Defaults to var.name. | `string` | `null` | no |
...
| <a name="input_create_browser"></a> [create\_browser](#input\_create\_browser) | When true, creates one Browser owned by this root module. | `bool` | `false` | no |
...
| <a name="input_create_evaluations"></a> [create\_evaluations](#input\_create\_evaluations) | When true, creates Evaluators and online evaluation configurations from this root module. | `bool` | `false` | no |
...
| <a name="input_create_gateway_connectors"></a> [create\_gateway\_connectors](#input\_create\_gateway\_connectors) | When true, creates the built-in connector targets declared in gateway\_connector\_targets. | `bool` | `false` | no |
...
| <a name="input_create_policy_engine"></a> [create\_policy\_engine](#input\_create\_policy\_engine) | When true, creates one Policy Engine shared by the enabled Gateways in this root module. | `bool` | `false` | no |
...
| <a name="input_gateway_policy_engine_mode"></a> [gateway\_policy\_engine\_mode](#input\_gateway\_policy\_engine\_mode) | When set, attaches the module-created Policy Engine to the primary Gateway in LOG\_ONLY or ENFORCE mode. | `string` | `null` | no |
| <a name="input_gateway_policy_templates"></a> [gateway\_policy\_templates](#input\_gateway\_policy\_templates) | Cedar policies rendered with the selected Gateway ARN plus caller-owned template values. gateway\_key is primary or an enabled additional Gateway key. | <pre>map(object({<br/>    gateway_key        = string<br/>    statement_template = string<br/>    template_values    = optional(map(string), {})<br/>    name               = optional(string)<br/>    description        = optional(string)<br/>    validation_mode    = optional(string, "FAIL_ON_ANY_FINDINGS")<br/>  }))</pre> | `{}` | no |
...
| <a name="input_image_digest"></a> [image\_digest](#input\_image\_digest) | Optional sha256 digest used with the module-created ECR repository. This keeps Runtime deployment immutable while create\_build\_pipeline is enabled. | `string` | `null` | no |
...
| <a name="input_online_evaluations"></a> [online\_evaluations](#input\_online\_evaluations) | Online evaluations. Set runtime\_key to primary or an enabled additional Runtime to derive its CloudWatch service source. | `any` | `{}` | no |
| <a name="input_policy_engine_description"></a> [policy\_engine\_description](#input\_policy\_engine\_description) | Description of the root Policy Engine. | `string` | `null` | no |
| <a name="input_policy_engine_name"></a> [policy\_engine\_name](#input\_policy\_engine\_name) | Name of the root Policy Engine. Defaults to var.name. | `string` | `null` | no |
...
| <a name="input_runtime_browser_access_enabled"></a> [runtime\_browser\_access\_enabled](#input\_runtime\_browser\_access\_enabled) | When true, grants the primary module-created Runtime role access to the module-created Browser. | `bool` | `false` | no |
...
| <a name="input_runtime_environment_bindings"></a> [runtime\_environment\_bindings](#input\_runtime\_environment\_bindings) | Environment variables resolved from resources in this module. source is memory\_id, browser\_id, or gateway\_url; gateway\_url requires key primary or an additional Gateway key. | <pre>map(object({<br/>    source = string<br/>    key    = optional(string, "primary")<br/>  }))</pre> | `{}` | no |
...
| <a name="input_runtime_memory_access_enabled"></a> [runtime\_memory\_access\_enabled](#input\_runtime\_memory\_access\_enabled) | When true, grants the primary module-created Runtime role access to the module-created Memory. | `bool` | `false` | no |
...
| <a name="input_runtime_resource_policy_configuration"></a> [runtime\_resource\_policy\_configuration](#input\_runtime\_resource\_policy\_configuration) | Optional IAM role allowlist for a resource policy attached to the primary Runtime. Use allow\_gateway\_role for the primary Gateway and gateway\_keys for additional Gateways. An empty effective role set creates an explicit deny-all policy. | <pre>object({<br/>    role_arns          = optional(set(string), [])<br/>    allow_gateway_role = optional(bool, false)<br/>    gateway_keys       = optional(set(string), [])<br/>  })</pre> | `null` | no |
...
| <a name="input_temporal_policy_templates"></a> [temporal\_policy\_templates](#input\_temporal\_policy\_templates) | Dogwood policies rendered with the selected Gateway ARN. gateway\_key is primary or an enabled additional Gateway key. | <pre>map(object({<br/>    gateway_key        = string<br/>    statement_template = string<br/>    template_values    = optional(map(string), {})<br/>    name               = optional(string)<br/>    description        = optional(string)<br/>    enforcement_mode   = optional(string, "LOG_ONLY")<br/>    validation_mode    = optional(string, "FAIL_ON_ANY_FINDINGS")<br/>  }))</pre> | `{}` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_additional_agent_runtimes"></a> [additional\_agent\_runtimes](#output\_additional\_agent\_runtimes) | Additional enabled Runtime instances keyed by caller-defined name. |
| <a name="output_additional_gateways"></a> [additional\_gateways](#output\_additional\_gateways) | Additional enabled Gateway instances keyed by caller-defined name. |
...
| <a name="output_browser_arn"></a> [browser\_arn](#output\_browser\_arn) | ARN of the module-created Browser, or null when disabled. |
| <a name="output_browser_id"></a> [browser\_id](#output\_browser\_id) | ID of the module-created Browser, or null when disabled. |
...
| <a name="output_policy_arns"></a> [policy\_arns](#output\_policy\_arns) | Cedar and temporal policy ARNs keyed by caller-defined name. |
| <a name="output_policy_engine_arn"></a> [policy\_engine\_arn](#output\_policy\_engine\_arn) | ARN of the module-created Policy Engine, or null when disabled. |
| <a name="output_policy_engine_id"></a> [policy\_engine\_id](#output\_policy\_engine\_id) | ID of the module-created Policy Engine, or null when disabled. |
...
### VPC mode (private endpoint)

Run the runtime inside your VPC so it never receives a public endpoint. Both `vpc_subnet_ids` and `vpc_security_group_ids` are required when `network_mode = "VPC"`:
...
module "agentcore" {
  ...
  vpc_security_group_ids = [aws_security_group.agent_runtime.id]
}
```

> **Note:** The subnets must have a route to the ECR, CloudWatch Logs, and Bedrock service endpoints — either via NAT Gateway or VPC Interface Endpoints.

### JWT-authenticated runtime endpoint

Protect the runtime endpoint with OIDC/JWT auth so only tokens from your IdP are accepted:
...
Set `network_mode = "VPC"` to prevent the runtime from receiving a public endpoint. When VPC mode is enabled, `vpc_subnet_ids` and `vpc_security_group_ids` are both required — the module validates this at plan time. Ensure the subnets have connectivity to:

- `ecr.dkr.<region>.amazonaws.com` and `ecr.api.<region>.amazonaws.com` (image pull)
- `logs.<region>.amazonaws.com` (CloudWatch Logs)
- `bedrock-agentcore.<region>.amazonaws.com` (runtime service)

Use NAT Gateway or VPC Interface Endpoints depending on your network topology.

### 🔐 JWT authorizer

Set `authorizer_discovery_url` to a valid OIDC discovery URL (must end with `/.well-known/openid-configuration`) to attach a JWT authorizer to the runtime endpoint. Use `authorizer_allowed_audience` and `authorizer_allowed_clients` to scope accepted tokens. When null (default), the endpoint is unauthenticated at the AgentCore layer.
